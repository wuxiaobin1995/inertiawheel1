 ...
 * @LastEditTime: 2020-12-26 14:48:25
 * @Description: 运动测试-时间模式
 -->
<template>
...
export default {
  ...
  data() {
    return {
      ...
      isOpened: false, // 串口open/close按钮切换
      ...
      SerialPort.list((err, ports) => {
        ...
        if (comPath) {
          ...
          this.parser.on('data', data => {
            ...
            if ((this.ChartData.length - 1) * 0.2 < this.GroupTime) {
              ...
              parseFloat(((this.ChartData.length - 1) * 0.2).toFixed(0)) ===
              this.GroupTime
            ) {
              if (this.F103Port.isOpen) {
                this.F103Port.close()
    ...
    readCurrentUser() {
      ...
      request.onsuccess = event => {
        ...
        readrequest.onsuccess = event => {
          ...
          this.GroupTime = readrequest.result[0].GroupTime
          this.countTime = this.GroupTime // 倒计时间
    ...
    openSerial() {
      ...
      if (!this.F103Port.isOpen) {
        ...
        this.interval = setInterval(() => {
          ...
          // 5秒以内，判断有无数据上来
          if (this.countTime === this.GroupTime - 5) {
            if (this.ChartData.length <= 5) {
              if (this.F103Port.isOpen) {
                this.F103Port.close()
              }
              this.$alert(
                '传感器没有数据传输到电脑，请重新连接USB线，再刷新页面！',
                '错误',
                {
